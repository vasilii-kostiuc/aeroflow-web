...
Панель допускает несколько прогонов одной карточки в день (задача 014). К четырём
действиям добавлен фильтр **«Новый запуск»**: он показывает карточки, чей последний
occurrence достиг финального статуса цикла (`boarding`/`arrival_announced`), и его
панель одной кнопкой вызывает `POST /api/v1/flight-occurrences:start-next-manual`
(серверный инкремент `sequenceNumber`). Защита от двойного клика — доменная: повтор
при ещё незавершённом прогоне возвращает `409`. Статус карточки берётся из
последнего occurrence за дату — dispatcher read model отдаёт по одной (последней)
записи на карточку.
